...
class list {
   ...
   public:
    ...
    void reorder(int pos, std::vector<int> selected) {
        std::vector<T> value;
        size_t i = 0, j = 0;
        iterator itrPos = begin();
        while (pos--) itrPos++;
        for (iterator itr = begin(); itr != end() && j < selected.size(); itr++, i++) {
            if (i == selected[j]) {
                value.push_back(itr.ptr->data), j++;
                itr = erase(itr);
                itr--;
            }
        }

        for (int i = 0; i < value.size(); i++) insert(itrPos, value[i]);
    }
};
}  // namespace CP
